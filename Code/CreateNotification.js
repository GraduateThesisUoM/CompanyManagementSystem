...
async function create_notification(userID, relevantUserID, notificationType){
    // check if a notification of the same type exists for user
    var exist_check = await Notification.findOne({$and: [{user_id: userID}, {type: notificationType}, {status: "unread"}]});
